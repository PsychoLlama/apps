...
- Token stories live in `@dev/storybook` (see `packages/dev/storybook/stories/design/`), not here, because they render with `@lib/ui` components and `@lib/ui` depends on `@lib/design`.
