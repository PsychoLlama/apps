...
/** Update the in-memory motion selection. */
export const setMotion = defineAction(
  [motionStore],
  (motion, id: MotionOption) => {
    motion.id = id;
  },
);
 ...
 * setter, so the store just learns what's already on screen.
 */
export const hydrateMotionEffect = defineEffect([], readActiveMotion, {
  onSuccess: setMotion,
});

/**
 * Switch the active motion override. Dispatches `setMotion` first so the
 * UI reacts synchronously, then flips `<html data-reduced-motion>` (or
 * drops it, for `'system'`) and updates localStorage so the prelude can
 * restore the choice before paint on the next load.
 */
export const selectMotionEffect = defineEffect([], applyMotion, {
  onStart: setMotion,
});
