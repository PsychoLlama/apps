 ...
 * `vitest.browser.config.ts` that re-exports {@link browserConfig}, run
 * under `chromium-lock`) so turbo caches and reruns browser tests per
 * package instead of booting Chromium for the whole monorepo on every
 * change. Both suites must
...
/**
 * A complete browser-suite config, re-exported as each package's
 ...
 * below only matches the invoking package's browser tests.
 */
